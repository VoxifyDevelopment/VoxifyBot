...
export default class context implements MessageContextMenuExecutor {
    id = (bot: VoxifyClient, localeName?: string) =>
        localeName ? bot.translations.translateTo(localeName, 'context.user.kick-user.name') : bot.translations.translate('context.user.kick-user.name');
    data(bot: VoxifyClient) {
        ...
        Object.entries(bot.translations.translations).forEach(([localeName, translation]) => {
            ...
            let name = bot.translations.translateTo(normalizedLanguage, 'context.user.kick-user.name');
    ...
    async execute(event: MessageContextMenuInteractionEvent) {
        ...
        const target = interaction.guild?.members.cache.get(interaction.targetMessage.author.id);
        ...
        target.voice.disconnect(`TempVoice | kick requested by [user ${member.user.username}]`).catch(console.error);
