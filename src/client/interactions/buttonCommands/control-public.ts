    ...
    async execute(event: ButtonCommandEvent) {
        ...
        if (
            !(await bot.tools.discord.checkTvcArgs(
                localeName,
                resolvedArgs,
                bot.translations.translateTo(localeName, 'buttons.public.name'),
                bot,
                interaction,
                true,
                true,
        ...
        const key = bot.translations.translateTo(localeName, 'buttons.public.name');
        const feedback = bot.translations.translateTo(localeName, 'feedback.success');
        const content = bot.translations.translateTo(localeName, 'buttons.public.success');
