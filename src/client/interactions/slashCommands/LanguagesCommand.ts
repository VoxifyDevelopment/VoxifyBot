...
export default class PingCommand implements SlashCommandExecutor {
    ...
    data = (bot: VoxifyClient) => {
        let dataJson = new SlashCommandBuilder().setName(this.name).setDescription(bot.translations.translate('commands.languages.description'));
        ...
        Object.entries(bot.translations.translations).forEach(([localeName, translation]) => {
            ...
            let description = bot.translations.translateTo(normalizedLanguage, 'commands.languages.description');
