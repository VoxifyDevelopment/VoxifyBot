...
export default class PingCommand implements SlashCommandExecutor {
    ...
    data = (bot: VoxifyClient) => {
        let dataJson = new SlashCommandBuilder().setName(this.name).setDescription(bot.translations.translate('commands.premium.description'));
        ...
        Object.entries(bot.translations.translations).forEach(([localeName, translation]) => {
            ...
            let name = bot.translations.translateTo(normalizedLanguage, 'commands.premium.name');
            names[normalizedLanguage as Locale] = name;
            let description = bot.translations.translateTo(normalizedLanguage, 'commands.premium.description');
    ...
    async execute(event: SlashCommandEvent) {
        ...
        const key = bot.translations.translateTo(localeName, 'commands.premium.name');
        const feedback = bot.translations.translateTo(localeName, 'feedback.success');
        const content = bot.translations.translateTo(localeName, 'commands.premium.success');
